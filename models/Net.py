...
class ProxyNet_sub(nn.Module):
    def __init__(self, backbone=None, use_GeM=False, branch_num=2, return_embedding=False):
        super(ProxyNet_sub, self).__init__()
        self.model_name = self.__class__.__name__
        self.use_GeM = use_GeM
        self.branch_num = branch_num
        ...
        setattr(self, 'embedder_x',
                BNClassifier(2048 * self.branch_num, 2048 * self.branch_num))
        # Embedder_BottleBlock(2048 * self.branch_num, 2048 * self.branch_num, relu=True, dropout=False,bottle_dim=1024))
    ...
    def forward(self, x):
        ...
        if self.use_GeM:
            x = (F.adaptive_avg_pool2d(x, (self.branch_num, 1)) + F.adaptive_max_pool2d(x,
                                                                                        (self.branch_num, 1))).squeeze()
        else:
            x = self.avgpool_MAP(x).squeeze()
        # x = self.dropout(x)
        # [bs,2048,branch_num]
        ...
        if self.return_embedding:
            ...
            embedding_x_pre = embedder_x(self.fallten(x))
            # embedding_x_pre = torch.randn_like(embedding_x_pre)
            return features, embedding_x_pre
        else:
            return features, x

        #  x : [bs,2048,branch_num]
        # embedding_x_pre : [bs,4096]
        # features [bs,2048,16,8]
...
class AnchorNet(nn.Module):
    def __init__(self, boneback , use_GeM=False, branch_num=2):
        ...
        self.core_net = ProxyNet_sub(use_GeM=self.use_GeM, branch_num=self.branch_num, return_embedding = False)
        self.supply_net = ProxyNet_sub(backbone = boneback, use_GeM=self.use_GeM, branch_num=self.branch_num, return_embedding = True)
    ...
    def forward(self,x):
        # fearues -- [bs,2048,16,8] ;
        # mmd_embedder_core -- [bs, 2048, branch_num]
        # mmd_embedder_supply -- [bs, 2048 * branch_num]

        features_core, mmd_embedder_core = self.core_net(x)
        features_supply, mmd_embedder_supply = self.supply_net(x)

        # 融合两个网络的特征
        # features_merger -- [bs,2048,16,8]
        features_merger = torch.cat([features_core, features_supply], dim=1)
        # features_merger = self.merger(torch.cat([features_core,features_supply],dim=1))
        # mmd_embedders -- [bs,2048, branch_num *2]
        feature1 = torch.cat([logits[:, :, i] for i in range(logits.size(2))], dim=1)
        mmd_embedders = torch.cat([mmd_embedder_core, mmd_embedder_supply],dim=2)

        return features_merger, mmd_embedders
...
if __name__ == '__main__':
    ...
    # result = adjust_net(temp)
    # print(result.shape)
    # net = Net_self()
    # print(summary(net, input_size=[(3, 256, 128)], batch_size=1, device='cpu'))
    # # temp = temp.cuda()
